...
def MAIN_EXTRACT(args):
    ...
    libtiff = "libtiff.so.5.2.6"
    ops.copyto(ops.path_join(src_usr_lib_dir, libtiff), dst_lib_dir)
    ops.ln(dst_lib_dir, libtiff, "libtiff.so.5")
    ops.ln(dst_lib_dir, libtiff, "libtiff.so")

    ops.mkdir(dst_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'tiffconf.h'), dst_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'tiff.h'), dst_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'tiffio.h'), dst_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'tiffvers.h'), dst_include_dir)
